// navData.js

export const navData = [
    {
        label: "Men",
        sections: [
            {
                title: "New & Featured",
                items: [
                    "Back to School: Extra 20% Off",
                    "New Arrivals",
                    "Best Sellers",
                    "Latest Drops",
                    "Shop Back to Sport",
                    "Fall Collection",
                    "SNKRS Launch Calendar",
                ],
            },
            {
                title: "Shoes",
                items: [
                    "All Shoes",
                    "Basketball",
                    "Lifestyle",
                    "Jordan",
                    "Retro Running",
                    "Running",
                    "Sandals & Slides",
                    "Shoes $100 & Under",
                    "Training & Gym",
                    "Sale Shoes",
                ],
            },
            {
                title: "Clothing",
                items: [
                    "All Clothing",
                    "Hoodies & Sweatshirts",
                    "Jordan",
                    "Jackets & Vests",
                    "Matching Sets",
                    "Pants",
                    "Shorts",
                    "Tops & Graphic Tees",
                    "24.7 Collection",
                    "Sale Clothing",
                ],
            },
            {
                title: "Accessories",
                items: [
                    "Bags & Backpacks",
                    "Belts",
                    "Hats & Headwear",
                    "Socks",
                    "Sunglasses",
                    "Underwear",
                    "Sale Accessories",
                ],
            },
        ],
    },
    {
        label: "Women",
        sections: [
            {
                title: "New & Featured",
                items: [
                    "Back to School: Extra 20% Off",
                    "New Arrivals",
                    "Best Sellers",
                    "Shop Back to Sport",
                    "Air Max Muse & Air Superfly",
                    "Fall Collection",
                    "SNKRS Launch Calendar",
                ],
            },
            {
                title: "Shop by Color",
                items: [
                    "Picante Red",
                    "Teal Tones",
                    "Pink Pops",
                    "Burgundy Crush",
                    "Midnight Navy",
                    "Timeless Neutrals",
                ],
            },
            {
                title: "Shoes",
                items: [
                    "All Shoes",
                    "Basketball",
                    "Lifestyle",
                    "Jordan",
                    "Retro Running",
                    "Running",
                    "Sandals & Slides",
                    "Shoes $100 & Under",
                    "Training & Gym",
                    "Sale Shoes",
                ],
            },
            {
                title: "Clothing",
                items: [
                    "All Clothing",
                    "Bras",
                    "Hoodies & Sweatshirts",
                    "Leggings",
                    "Matching Sets",
                    "Jackets & Vests",
                    "Pants",
                    "Skirts & Dresses",
                    "Shorts",
                    "Tops & Graphics Tees",
                    "24.7 Collection",
                    "Sale Clothing",
                ],
            },
            {
                title: "Accessories",
                items: [
                    "Bags & Backpacks",
                    "Belts",
                    "Hats & Headwear",
                    "Socks",
                    "Sunglasses",
                    "Sale Accessories",
                ],
            },
        ],
    },
    {
        label: "Kids",
        sections: [
            {
                title: "Shoes",
                items: [
                    "Basketball",
                    "Lifestyle",
                    "Jordan",
                    "Retro Running",
                    "Running",
                    "Sandals & Slides",
                    "Shoes $80 & Under",
                    "Soccer",
                    "Sale Shoes",
                ],
            },
            {
                title: "Clothing",
                items: [
                    "Big Kids (7-15 yrs)",
                    "Little Kids (3-7 yrs)",
                    "Baby & Toddler (0-3 yrs)",
                    "Back to School Clothing",
                    "Bras",
                    "Hoodies & Sweatshirts",
                    "Jordan",
                    "Matching Sets",
                    "Jackets & Vests",
                    "Pants & Tights",
                    "Shorts",
                    "Skirts & Dresses",
                    "Tops & T-Shirts",
                    "Sale Clothing",
                ],
            },
            {
                title: "Accessories",
                items: [
                    "Bags & Backpacks",
                    "Belts",
                    "Hats & Headwear",
                    "Socks",
                    "Sunglasses",
                    "Underwear",
                    "Sale Accessories",
                ],
            },
            {
                title: "Shop By Sport",
                items: [
                    "Basketball",
                    "Belts",
                    "Gymnastics",
                    "Football",
                    "Running",
                    "Soccer",
                ],
            },
        ],
    },
    {
        label: "Jordan",
        sections: [
            {
                title: "New and Feature",
                items: [
                    "Back to School: Extra 20% Off",
                    "New Arrivals",
                    "Best Sellers",
                    "Jordan Heat Check",
                    "Jordan Sleeper Picks",
                    "Shop Back to Sport",
                    "Shattered Backboard Capsule",
                ],
            },
            {
                title: "Accessories",
                items: ["Shop All", "Bags & Backpacks", "Hats & Headwear"],
            },
            {
                title: "Sport",
                items: [
                    "Jordan Basketball",
                    "Jordan Golf",
                    "Jordan Cleats",
                    "Women's Jordan Training & Gym",
                ],
            },
            {
                title: "Men",
                items: ["Shop All", "Shoes", "AJ1", "Clothing"],
            },
            {
                title: "Women",
                items: [
                    "Jordan Brand Women's Collection",
                    "Shop All",
                    "Shoes",
                    "AJ1",
                    "Clothing",
                ],
            },
            {
                title: "Kids",
                items: [
                    "Shop All",
                    "Shoes",
                    "AJ1",
                    "Clothing",
                    "Big Kids",
                    "Little Kids",
                    "Baby & Toddler",
                ],
            },
        ],
    },
    {
        label: "Sport",
        sections: [
            {
                title: "Basketball",
                items: [
                    "Shoes",
                    "Apparel",
                    "Equipment",
                    "Kobe",
                    "Jordan",
                    "NBA Gear",
                    "WNBA Gear",
                    "NCAA Gear",
                ],
            },
            {
                title: "Training",
                items: ["Shoes", "Socks", "Apparel", "Equipment"],
            },
            {
                title: "Running",
                items: [
                    "Road",
                    "Race",
                    "Trail",
                    "Track & Field",
                    "Apparel",
                    "Equipment",
                    "Stride & Swift Collection",
                    "Running Shoe Finder",
                ],
            },
            {
                title: "Football",
                items: ["Cleats", "Apparel", "Equipment", "NFL"],
            },
        ],
    },
    {
        label: "Sale",
        sections: [
            {
                title: "Back to School Sale",
                items: ["Extra 20% Off Select Styles"],
            },
            {
                title: "All Sale",
                items: ["Sale Shoes", "Sale Clothing", "Sale Accessories"],
            },
            {
                title: "Men",
                items: ["Shoes", "Clothing", "Accessories", "Shop All"],
            },
            {
                title: "Women",
                items: ["Shoes", "Clothing", "Accessories", "Shop All"],
            },
            {
                title: "Kid",
                items: ["Shoes", "Clothing", "Accessories", "Shop All"],
            },
        ],
    },
];
